...
export function createCard(card: Card, themeConfig: ThemeConfig): HTMLElement {
    ...
    const cardBack = card.asset
        ? `<img class="card__icon" src="${card.asset}" alt="${card.alt ?? card.label ?? ""}" onerror="this.style.display='none'; if(this.nextElementSibling) this.nextElementSibling.style.display='block';">
